      ...
      <div className="p-4 space-y-1">
        ...
        <div className="flex flex-wrap justify-between gap-2">
          ...
          {lastUpdate != null && lastUpdate > 0 && (
            <span className="inline-flex items-center gap-1 text-xs bg-amber-100 text-amber-800 rounded-full px-2.5 py-1">
              <TimeAgo ts={lastUpdate} />
            </span>
          )}
        </div>
...
export const ResultItem = ({
  ...
  return (
    <Link
      ...
      to={`/product/${item.id}`}
      key={`item-${item.id}`}
      className="group bg-white md:shadow-xs hover:shadow-md transition-all duration-300 relative snap-start flex-1 min-w-64 flex flex-col result-item hover:bg-linear-to-br hover:from-white hover:to-gray-50 border-b border-gray-200 md:border-b-0"
      onClick={trackItem}
    >
